...
class FastCNER:
	...
	def processRules(self, text, rule_map, match_begin, match_end, current_position,
		...
		if current_position < length:
			...
			if '\\' in rule_map:
				self.processWildCards(text, rule_map['\\'], match_begin, match_end, current_position, matches, pre_char, True, '\\')
			if '(' in rule_map:
				self.processRules(text, rule_map['('], current_position, match_end, current_position, matches, pre_char, False, '(')
			if ')' in rule_map:
				self.processRules(text, rule_map[')'], match_begin, current_position, current_position, matches, pre_char, False, ')')
			...
			if this_char in rule_map and this_char != ')' and this_char != '(':
				self.processRules(text, rule_map[this_char], match_begin, match_end, current_position + 1, matches, this_char, False, this_char)

			if self.support_replication and '+' in rule_map:
				self.processRules(text, rule_map['+'], match_begin, match_end, current_position, matches, this_char, False, '+')
				self.processReplication(text, rule_map['+'], match_begin, match_end, current_position, matches, this_char, wildcard_support, pre_key)
			...
			if FastCNER.END in deter_rule:
				...
				self.processRules(text, deter_rule['\\'], match_begin, current_position, current_position, matches, pre_char, False, ' ')
		elif current_position == length and '+' in rule_map:
			self.processRules(text, rule_map['+'], match_begin, match_end, current_position, matches, pre_char, wildcard_support, pre_key)
		pass
	...
	def processReplication(self, text, rule_map, match_begin, match_end, current_position, matches,
		...
		this_char = text[current_position]
		if wildcard_support:
			self.replication_funcs[pre_key](text, rule_map, match_begin, match_end, current_position, matches, this_char, pre_char)
		else:
			processReplicationCommon(lambda char: char == pre_key, self.processRules, text, rule_map, match_begin, match_end, current_position,
									 matches,
									 this_char, pre_char)
		pass
